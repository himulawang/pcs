!function () {
    ...
    ListStore.prototype.del = function del(list, cb) {
        if (list instanceof this.getListModel() === false) return cb(new I.Exception(10108, err));
        var self = this;
    ...
    ListStore.prototype.update = function update(list, cb) {
        if (list instanceof this.getListModel() === false) return cb(new I.Exception(10138, err));
        var self = this;
        var util = I.Util;
        var dbMulti = this.db.multi();
    ...
    ListStore.prototype.updateSync = function updateSync(list, cb) {
        if (list instanceof this.getListModel() === false) return cb(new I.Exception(10142, err));
        cb = cb || function() {};
        var self = this;
        var util = I.Util;
        var dbMulti = this.db.multi();
        ...
        list.toUpdateSyncList.forEach(function(index) {
            ...
            var toDB = child.toUpdate(true);
            if (util.getLength(toDB) === 0) return;
            dbMulti.hmset(childModel.abb + child.getPK(), toDB);
        });
        ...
        list.toAddSyncList.forEach(function(index) {
            ...
            dbMulti.hmset(childModel.abb + index, toDB);
            dbMulti.hset(self.abb + list.getPK(), child.getPK(), 1);
        });
        list.toAddSyncList = [];
